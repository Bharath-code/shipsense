...
describe('determineTasks', () => {
	it('generates a commit task if commit gap is greater than 24', () => {
		const tasks = determineTasks(true, 48); // private repo, 48 hr gap
	...
	it('generates an issue task if repo is public', () => {
		const tasks = determineTasks(false, 12); // public repo, 12 hr gap
		expect(tasks).toHaveLength(1);
	...
	it('generates both tasks if public and commit gap > 24', () => {
		const tasks = determineTasks(false, 48); // public repo, 48 hr gap
	...
	it('generates no tasks if private and commit gap <= 24', () => {
		const tasks = determineTasks(true, 12); // private repo, 12 hr gap
