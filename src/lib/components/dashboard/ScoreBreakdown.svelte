...
<div class="overflow-hidden rounded-[2rem] border glass-panel shadow-2xl">
	<button
		type="button"
		class="flex w-full items-center justify-between p-6 text-left transition-colors hover:bg-muted/20"
	...
	>
		...
		<div class="flex items-center gap-4">
			...
			{/if}
			<div
				class="flex h-8 w-8 items-center justify-center rounded-full bg-muted transition-transform {isExpanded
					? 'rotate-180'
					: ''}"
			>
	...
	{#if isExpanded}
		<div id="score-breakdown-content" class="border-t border-muted px-6 pb-6">
			{#if isLoading}
