...
- [x] Add competitor comparison to weekly email digest ("Your repo grew X%, [competitor] grew Y%")
...
- [x] Add plan gating: Indie = 1 report/mo, Builder = unlimited (tracked via userProfiles.investorReportsUsedThisMonth)
- [x] Add "Generated by ShipSense" watermark on free/indie tier reports

### One-Click Actions (P1 — dashboard → agent)

- [x] Create `convex/issueReplyDraft.ts` — AI-drafted issue responses via Gemini
- [x] Add `issueNumber` field to repoTasks schema
- [x] Update Tasks tab to show "✨ Generate Draft" button + expandable draft text
- [x] Add "📋 Copy" and "↗ Open on GitHub" buttons to draft
- [ ] Plan gating: Free = no drafts, Indie = 5/mo, Builder = unlimited (deferred — works for all users now)

### AI Issue Prioritization (P2 — burnout antidote)

- [x] Create `convex/issueClassification.ts` — classify recent issues during sync via Gemini
- [x] Wire classification into orchestrator sync pipeline (runs before task generation)
- [x] Add `urgency` field to repoTasks (critical/high/medium/low)
- [x] Add `storeClassifiedIssues` mutation creates tasks with urgency badges
- [x] Show urgency badges in Tasks tab (🔴 Critical, 🟡 High, etc.)
- [x] Overview tab shows "Critical issue needs attention" alert when urgency=critical

### Landing Page GTM Readiness

- [x] Add testimonial cards section (3 placeholder testimonials with star ratings)
- [x] Add "Join X builders" count to hero section when totalTracked > 10
